...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listaDeSolicitudes` ()  SQL SECURITY INVOKER
BEGIN
SELECT
o.numeroDeOrden AS 'Orden',
date_format(o.fechaOrden,'%d-%m-%Y') AS 'Fecha',
DATEDIFF(CURDATE(), o.fechaOrden) as 'Dias',
o.nombreDelCliente AS 'Cliente',
CONCAT('Telefono:',o.telefonoDelCliente,' Celular: ',o.celularDelCliente) AS 'Contacto',
CONCAT(o.descripcionDelProducto,' ',o.marcaDelProducto,' Mod.',o.modeloDelProducto,' Serie:',o.serieDelProducto) AS 'Producto',
o.fallaProducto AS 'Falla',
o.diagnostico AS 'Diagnostico',
o.trabajoRealizado AS 'Trabajo a realizar',
o.estado AS 'Estado',
s.Subtotal,
s.Iva,
s.Total
FROM
ordendeservicio o LEFT JOIN (SELECT p.numeroDeOrden, FORMAT(SUM(p.subtotal),2) AS 'Subtotal', FORMAT(SUM(p.subtotal)*(c.porcentajeDeIva/100),2) AS 'Iva', FORMAT(SUM(p.subtotal)+(SUM(p.subtotal)*(c.porcentajeDeIva/100)),2) AS 'Total' FROM presupuesto p, configuracion c GROUP BY p.numeroDeOrden) s
ON o.numeroDeOrden = s.numeroDeOrden
